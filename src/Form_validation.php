...
class Form_validation {
	...
	protected function _execute($row, $rules, $postdata = NULL, $cycles = 0)
	{
		...
		if (is_array($postdata) && ! empty($postdata) && isset($postdata[0]))
		{
		...
		foreach ($rules as $rule)
		{
			...
			$this->_cur_postdata = $postdata;
			$this->_cur_postdata_index = $cycles;
			$this->_cur_postdata_rule = $rule;
	...
	protected $_cur_postdata;
	protected $_cur_postdata_index;
	protected $_cur_postdata_rule;
	protected $_array_rules;

	public function set_array_rules($field, $label = '', $rules = 'required') {
		$rules = preg_split('/\|(?![^\[]*\])/', $rules);

		$this->_array_rules[$field] = array(
			'field' => $field,
			'label' => $label,
			'rules' => $rules,
		);
	...
	public function exec_array_rules() {
		$postdata = $this->_cur_postdata;
		$index = $this->_cur_postdata_index;
		$rules = $this->_array_rules;
		$arr_rule = $this->_cur_postdata_rule;
		$row_index = $index + 1;

		if (!$postdata) {
			$this->set_message($arr_rule, 'Tidak ada isian {field}');
			return false;
		}
		...
		foreach ($rules as $field => $value) {
			...
			foreach ($value['rules'] as $rule) {
				...
				if (!method_exists($this, $rule)) {
					$result = ($param !== FALSE) ? $rule($data, $param) : $rule($data);
				} else {
				...
				if ($result === FALSE) {
					...
					if (is_string($rule)) {
						$line = $this->_get_error_message($rule, $field);
						$message = "Kesalahan pada baris ke $row_index. ";
						$message .= $this->_build_error_msg($line, $this->_translate_fieldname($label), $param);
					}

					$this->set_message($arr_rule, 'Isian {field} masih belum benar. ' . $message);
